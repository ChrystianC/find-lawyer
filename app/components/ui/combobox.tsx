...
export function MyCombobox(props: {
  mapLocation: Locations[];
  mapSpecialization: Specializations[];
  choosenLocation?: Locations;
  choosenSpecialization?: Specializations;
}) {
  const {
    mapLocation,
    mapSpecialization,
    choosenLocation,
    choosenSpecialization,
  } = props;
  const [selectedLocation, setSelectedLocation] = useState(
    choosenLocation ?? { idLocation: "", location: "" }
  );
  const [selectedSpecialization, setSelectedselectedSpecialization] = useState(
    choosenSpecialization ?? { idSpecialization: "", specialization: "" }
  );
  ...
  return (
    <>
      <div className="relative flex-grow w-full">
        <label className="absolute bottom-10 leading-7 text-sm text-gray-300">
          Specialization
        </label>
        <Combobox
          value={selectedSpecialization}
          onChange={setSelectedselectedSpecialization}
        >
          <div className="relative mt-1">
                    ...
                    <Combobox.Option
                      ...
                      className={({ active }) =>
                        `relative cursor-pointer select-none py-2 pl-10 pr-4 ${
                          active ? "bg-gray-900 text-white" : "text-white-900"
                        }`
                      }
                      value={option}
                    >
      ...
      <div>
        <button
          className="flex justify-center items-center h-9 w-32 border-5 border-gray-800 text-gray-200 font-semibold hover:bg-indigo-700 py-2 px-4 border rounded hover:border-blue-900 disabled:hover:border-rose-900  disabled:hover:bg-rose-800 bg-indigo-800"
          disabled={disabled}
          onClick={() => {
            return (
              router.replace(
                `/components/cards?${createQueryString("idSpecialization", selectedSpecialization.idSpecialization)}/${createQueryString("idLocation", selectedLocation.idLocation)}`
              ),
              undefined,
              { shallow: true }
            );
