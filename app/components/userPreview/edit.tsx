...
export default function UserPreview ( { Useremail, Userpassword, UseruserName, userId, appoiments, userCookie, authCookie } )
{
    const router = useRouter();
    const [ password, setPassword ] = useState( Userpassword ?? '' );
    const [ email, setEmail ] = useState( Useremail ?? '' );
    const [ userName, setUserName ] = useState( UseruserName ?? '' );
    const renderAppoments = appoiments.map( async ( { date, createdAt, service } ) =>
    {
        return ( <div className=" bg-gray-200 bg-opacity-75 px-8 pt-6 pb-6 rounded-lg overflow-hidden text-center relative m-5">
            ...
            <p className="leading-relaxed mb-3">Created by: { email } { createdAt.toDateString() }</p>
        </div> );
    ...
    return ( <div className='bg-gray-50'>
        ...
        <form onSubmit={ handleSubmit }>
            ...
            <div className="container px-5 py-16 mx-auto ">
                ...
                <button type='submit' className="flex mx-auto mt-20 text-white bg-gray-700 border-0 py-2 px-8 focus:outline-none hover:bg-gray-900 rounded text-lg">Change Data</button></div>
        </form>
    </div>
    );
}
    ...
    return (
        <header className='text-gray-600 border-b border-gray-700 bg-gray-900'>
            <div className='container mx-auto flex flex-col p-5 lg:flex-row items-center'>
                ...
                <button onClick={ () => deleteCokie( userCookie, authCookie ) } className='border-5 border-transparent inline-block hover:border-rose-900 text-gray-500 font-semibold hover:text-white py-2 px-4 border rounded'>Log out</button>
            </div>
        </header>
    );
};