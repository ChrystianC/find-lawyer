...
export default async function AppoimentsList({
  appoiments,
  idOffice,
  rerender,
}) {
  ...
  const renderAppoments = await appoiments.map(
    ({ idAppointment, date, isBooked, service }) => (
      <div
        className="border-b pb-4 border-gray-400 border-dashed"
        key={idAppointment.toString()}
      >
        <p className="text-xs text-center font-light leading-3 text-gray-500 dark:text-gray-300">
          Meeting
        </p>
        <BookApoiment
          date={date}
