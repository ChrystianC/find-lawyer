...
export default async function CardPreview() {
  ...
  const user = idUser
    ? await prisma.user.findFirst({ where: { idUser: { equals: idUser } } })
    : undefined;
  const isNotOffice = cookieStore.get("office")?.value === undefined;

  const appoiments = await prisma.appointments.findMany({
    where: { IdOffice: lawOffice.idLawOffice },
  });
  const renderAppoments = appoiments.map(
    async ({ date, createdAt, service, idCustomer }) => {
      const customer = idCustomer
        ? await prisma.user.findFirst({ where: { idUser: idCustomer } })
        : undefined;
      ...
      return (
        <div className=" bg-gray-100 bg-opacity-75 px-8 pt-6 pb-6 rounded-lg overflow-hidden text-center relative m-5">
          ...
          <p className="leading-relaxed mb-3">
            Created by: {customer?.email} {createdAt.toDateString()}
          </p>
        </div>
      );
    }
  ...
  const editCard = async (
    ...
    if (
      ...
      lawOfficeName === lawOffice.lawOfficeName &&
      officeSpecialization === lawOffice.officeSpecialization &&
      profile === lawOffice.profile
    )
      return;
  ...
  return (
    <div className="bg-slate-50">
      {!cookieStore.get("office")?.name ? (
        ...
        <Logout
          officeCookie={cookieStore.get("office")}
      ...
      <div className="container px-5 py-16 mx-auto">
        <div className="flex flex-wrap w-full mb-20 flex-col items-center text-center">
          {lawOffice.profile ? (
            <img
              src={lawOffice.profile}
              className={`w-60 h-60 rounded-full inline-flex items-center justify-center bg-gray-200 text-gray-400  border-gray-800 border-8`}
            />
          ) : (
            <span
              className={`w-60 h-60  rounded-full inline-flex items-center justify-center bg-gray-200 text-gray-400 text-5xl`}
            >
              ?
            </span>
          )}
          <h1 className="sm:text-4xl text-3xl font-medium title-font mb-2 text-gray-900 mt-2">
            Law Office Editor Page
          </h1>
          <p className="lg:w-1/2 w-full leading-relaxed text-gray-500">
            Enhance Your Law Office Efficiency and Accuracy: Discover Superior
            Document Editing Solutions for Legal Professionals
          </p>
        </div>
        <EditForm
          edit={editCard}
          ...
          newAppoiment={addAppoiment}
          mapLocation={locations}
          mapSpecialization={specializations}
          isNotOffice={isNotOffice}
        />
        ...
        <div className="flex flex-wrap w-full mb-20 flex-col items-center text-center mt-5">
          ...
          <p className="lg:w-1/2 w-full leading-relaxed text-gray-500">
            Join the Conversation: Share Your Thoughts, Insights, and
            ...
            <div className="border border-gray-200 p-6 rounded ">
              <div className="w-10 h-10 flex justify-center items-center rounded-full bg-gray-700  mb-4">
                <svg
                  ...
                  fill="currentColor"
                  className="block w-4 h-4 text-gray-200"
