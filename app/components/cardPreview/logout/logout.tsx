...
export default function Logout ( { officeCookie, authCookie } )
{
    ...
    return <>{ officeCookie ? <header className='text-gray-600 border-b border-gray-700 bg-gray-900'>
        <div className='container mx-auto flex flex-col p-5 lg:flex-row items-center'>
            ...
            <button onClick={ () => deleteCokie( officeCookie, authCookie ) } className='border-5 border-transparent inline-block hover:border-rose-900 text-gray-500 font-semibold hover:text-white py-2 px-4 border rounded'>Log out</button>
        </div>
    </header> : <></> }</>;
}