...
export default function UserPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) setPasswordNotValid(() => data);
      if (response.ok) router.replace("/components/login/user");
    } catch (e) {
      console.log(e);
    }
  };
  const checkIfPasswordIsValid = (pass) => {
    if (pass.length === 0) return;
    return (
      <h2 className="text-rose-900 text-lg title-font font-medium mb-2">
            ...
            <div className="flex-grow sm:text-left text-center mt-6 sm:mt-0">
              ...
              <div className="relative flex-grow w-full">
                <input
                  ...
                  className="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-transparent focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out"
                />
                {checkIfPasswordIsValid(passwordNotValid)}
              </div>
            </div>
          </div>
          <button
