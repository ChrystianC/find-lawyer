...
export default async function LawOfficeCardsPage() {
  ...
  const emailOffice = office
    ? await prisma.lawOffice.findUnique({ where: { email: office?.value } })
    : undefined;
  const requestUrl = headers().get("x-url").split("?")[1].split("%2F");
  const [singleSpecialization, singleLocation] = await Promise.all([
    getSpecializationFromParam(requestUrl[0]),
    getLocationFromParam(requestUrl[1]),
  ]);
  const [specializations, locations] = await Promise.all([
    getSpecializations(),
    getLocations(),
  ]);
  const getLawOffice = await getLawOffices(
    singleLocation,
    singleSpecialization
  );
  const renderLawOffice = getLawOffice.map(
    async ({
      idLawOffice,
      profile,
      lawOfficeName,
      ...
      const [rating, service] = await Promise.all([
        getRatings(idLawOffice),
        getService(idLawOffice),
      ]);
      const apoimentsArray = !!cookieStore.get(`name-${idLawOffice}`)?.value
        ? await getAppointmentsList(
            JSON.parse(cookieStore.get(`name-${idLawOffice}`).value),
            idLawOffice
          )
        : [];
      return (
        <div className="container  px-5 py-5 mx-auto min-h-vh">
          <div className="lg:w-4/6 relative mx-auto flex flex-wrap border-2 p-5 shadow-md bg-gray-100">
              ...
              <div className="flex items-center justify-center py-4 px-4">
                <div className="max-w-sm w-full shadow-lg overflow-y-auto lg:max-h-72 max-h-60">
                  ...
                  <div className="pt-2 md:px-16 px-5 dark:bg-gray-700 bg-gray-50 rounded-b">
                    <div className="px-4">
                      <AppoimentsList
                        appoiments={apoimentsArray}
                        idOffice={idLawOffice}
                        rerender={headers().get("x-url")}
                      />
                    </div>
                  </div>
                </div>
            ...
            <div className="absolute right-10 lg:top-5 top-72"></div>
            <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
              <div className="flex flex-row items-center">
                {profile ? (
                  ...
                  <span
                    className={`w-20 h-20 rounded-full inline-flex items-center justify-center bg-gray-200 text-gray-400 text-5xl`}
                  >
                    ?
                  </span>
                )}
                <div className="flex flex-col ml-2">
                  ...
                  <h1 className="text-gray-900 text-3xl title-font font-medium mb-1 hover:text-indigo-500">
                    {auth ?? email === emailOffice?.email ? (
                      <Link href={`/components/cardPreview?${idLawOffice}`}>
                        {lawOfficeName}
                      </Link>
                    ) : (
                      <span>{lawOfficeName}</span>
                    )}
                  </h1>
                </div>
              </div>
              <div className="mb-5 text-sm items-center flex mt-0">
                ...
                <span className="ml-2 pl-2 py-2 border-l-2 border-gray-200 space-x-2s">
                  <a
                    href={`https://www.google.com/maps/search/?api=1&query=${city}%20${address}`}
                  >
                    {address}, {city}
                  </a>
                </span>
  ...
  return (
    <>
      <header className="bg-gray-800">
        <div className="container flex-col lg:flex-row flex flex-grow sm:w-3/4 sm:space-x-4 sm:space-y-0 space-y-4 p-10 lg:items-center">
          <div className="flex items-center justify-center lg:justify-stretch">
            ...
            />
            <a className="ml-3 text-xl text-white lg:text-gray-800" href={"/"}>
              Lawyler
            </a>
          </div>
          <MyCombobox
            ...
            choosenLocation={singleLocation}
            choosenSpecialization={singleSpecialization}
          />
          {auth !== undefined ? (
            <div className="absolute right-16 text-gray-200">
              <Link href={"/"}>
                <svg
      ...
      <div className="bg-slate-100 min-h-96">{renderLawOffice}</div>
    </>
  );
}
