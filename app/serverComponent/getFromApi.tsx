...
export async function getSpecializationFromParam(params: string) {
  ...
  const specialization: Specializations =
    await prisma.specializations.findFirst({
      where: { idSpecialization: { equals: paramIdSpecialization } },
    });
...
export async function getLocationFromParam(params: string) {
  ...
  const location: Locations = await prisma.locations.findFirst({
    where: { idLocation: { equals: paramIdLocation } },
  });
  return location;
}

export async function getLawOffices(
  choosenlocation: Locations,
  choosenSpecialization: Specializations
) {
  const lawOffices: LawOffice[] = await prisma.lawOffice.findMany({
    where: {
      officeSpecialization: { equals: choosenSpecialization.specialization },
      city: { equals: choosenlocation.location },
    },
  });
  return lawOffices;
}
export async function getRatings(officeId: string) {
  const reviews: Ratings[] = await prisma.ratings.findMany({
    where: { idOffice: { equals: officeId } },
  });
  return reviews;
}
...
export async function getSpecializations() {
  const specializations: Specializations[] =
    await prisma.specializations.findMany();
...
export async function getAppointment(officeId: string) {
  const reviews: Appointments[] = await prisma.appointments.findMany({
    where: { IdOffice: { equals: officeId } },
  });
  return reviews;
}
